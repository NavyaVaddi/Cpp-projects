...
struct{
   char name[60];
   int acc_no, age; // acc_no -> account number
   char address[60];
   char citizenship[15];
   double phone;
   char acc_type[10];
   float amount;
   struct date dob; // do -> date of birth
   struct date deposit;
...
void new_account()
{
   ...
   while(fscanf(ptr,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d\n", &add.acc_no,add.name, &add.dob.month, &add.dob.day, &add.dob.year, &add.age,add.address,add.citizenship, &add.phone,add.acc_type, &add.amount, &add.deposit.month, &add.deposit.day,&add.deposit.year)!=EOF)
   {
      if(check.acc_no == add.acc_no){
         printf("Account n0. already in use!");
         fordelay(1000000000);
          goto account_no;
      }
   }
   ...
   fprintf(ptr,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d\n", add.acc_no, add.name,add.dob.month, add.dob.day, add.dob.year, add.age, add.address, add.citizenship, add.phone, add.acc_type, add.amount, add.deposit.day, add.deposit.month, add.deposit.year);
  ...
  void transact(void)
  {
     ...
     while(fscanf(old,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d", &add.acc_no,add.name, &add.dob.month, &add.dob.day, &add.dob.year, &add.age,add.address,add.citizenship, &add.phone,add.acc_type, &add.amount, &add.deposit.month, &add.deposit.day,&add.deposit.year)!=EOF)
     {
        if(add.acc_no == transaction.acc_no){
           ...
           {
              printf("\a\a\a\n\nYOU CANNOT DEPOSIT OR WITHDRAW CASH IN FIXE ACCOUNTS!!!");
              fordelay(1000000000);
              system("cls");
              menu();
           }
       ...
       while(fscanf(ptr,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d", &add.acc_no,add.name, &add.dob.month, &add.dob.day, &add.dob.year, &add.age,add.address,add.citizenship, &add.phone,add.acc_type, &add.amount, &add.deposit.month, &add.deposit.day,&add.deposit.year)!=EOF)
       {
          if(add.acc_no==check.acc_no){
             ...
             if(strcmpi(add.acc_type,"fixed1")==0){
                time=1.0;
                rate=9;
                interst=interest(time,add.amount,rate);
                printf("\n\nYou will get $%.2f as interest on %d/%d/%d", interest,add.deposit.month,add.deposit.day,add.deposit.year+1);
             }
             else if(strcmpi(add.acc_type,"fixed2")==0){
                time=2.0;
                rate=11;
                interst=interest(time,add.amount,rate);
                printf("\n\nYou will get $%.2f as interest on %d/%d/%d", interest,add.deposit.month,add.deposit.day,add.deposit.year+2);
             }
             if(strcmpi(add.acc_type,"fixed3")==0){
                time=3.0;
                rate=13;
                interst=interest(time,add.amount,rate);
                printf("\n\nYou will get $%.2f as interest on %d/%d/%d", interest,add.deposit.month,add.deposit.day,add.deposit.year+3);
             }
             else if(strcmpi(add.acc_type,"saving")==0){
                time=(1.0/12.0);
                rate=8;
                interst=interest(time,add.amount,rate);
                printf("\n\nYou will get $%.2f as interest on %d of every month", interest,add.deposit.day);
             }
             else if(strcmpi(add.acc_type,"curent")==0){
...
int main()
{
    char pass[20],password[20]="abcd";
    int i=0;
    printf("\n\n\t\tEnter the password to login:");
    scanf("%s",pass);
    /* do{if(pass[i]!=13 && pass[i]!=8){
           printf("*"); pass[i]=getch();i++; }}
        while(pass[i]!=13); pass[10]='\0';*/
